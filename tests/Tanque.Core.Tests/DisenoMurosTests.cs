...
/// módulo: el año de revisión se confirmaron y corrigieron dos observaciones de seguridad estructural
...
/// de esos observaciones, en su segunda ronda de auditoría (O1), mostró que un artefacto de
...
/// nunca cerrado (ya señalado como pendiente en la hoja de seguimiento del proyecto, "seis
/// módulos sin pruebas xUnit espejo"). Escrito como parte de una auditoría interna proactiva del
/// propio proyecto tras la reprimenda explícita del usuario del año de revisión sobre la gravedad de
...
/// muro" y la corrección de sismo fuera de dominio, año de revisión).
///
/// LIMITACIÓN HONESTA, igual que el resto de los espejos xUnit de este proyecto: este archivo NO
/// se pudo compilar ni ejecutar con `dotnet test` real (nuget.org bloqueado en el entorno de desarrollo de
...
public class DisenoMurosTests
{
    ...
    private static Geometria GeoSintDentroDominio() => new(
        ...
        // Em=0.50 (no 0.25): con la cota conservadora corregida Mx=q·a²/2 (observación año de revisión,
    ...
    public void DiametroInsuficiente_MarcaNoCumpleYSugiereElDiametroSuperior()
    {
        ...
        // (DetalladoInsuficiente) y sugerir No.5 (15.9 mm) -- blindaje año de revisión.
        var mat = MatSint();
        var ec = 4700.0 * Math.Sqrt(mat.FcMPa);
    ...
    {
        // Cruce normativo C.23-C.14.3 (año de revisión): el refuerzo de MURO de tanque usa la cuantía
    ...
    {
        // Cruce normativo C.23-C.14.3.3 (año de revisión): el refuerzo HORIZONTAL de muro usa la cuantía
    ...
    // ---- Corrección P1-P5/R1-R5 (sismo fuera del dominio tabulado, año de revisión): extremo a extremo ----
    ...
    public void FueraDeDominio_DisenarMuroLongitudinal_YaNoOmiteElSismo()
    {
        ...
        // H2 (año de revisión): el motivo NO debe filtrar el mensaje crudo de una excepción .NET interna.
    ...
    {
        // Observación confirmado año de revisión vía prueba real de usuario: TipoTanque.Superficial (Hm=0

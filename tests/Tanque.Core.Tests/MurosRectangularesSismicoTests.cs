...
/// este módulo (no existía ninguno antes de la corrección del año de revisión; a diferencia del fix de
/// flexión, esto no era un espejo desincronizado, era un hueco de cobertura nunca cerrado, ver
/// la hoja de seguimiento del proyecto). Escrito inmediatamente después de las correcciones
...
public class MurosRectangularesSismicoTests
{
    ...
    public void FueraDeDominio_FormulasCerradasIndependientes_CantiliverMasFranja()
    {
        ...
        var tab = MurosRectangularesSismico.Calcular(3.0, cSobreA, qInterior, a).LadoLargo; // REFINAMIENTO año de revisión: cota tabulada de banda
        ...
        var mxEsperado = Math.Min(qInterior * a * a / 2.0, mxTab); // cantiléver UNIFORME q*a^2/2 (observación año de revisión), acotado por la fila b/a=3.0
    ...
    public void FueraDeDominio_CotaConservadora_EnLimiteDeDominio_EsSiempreMayorOIgualQueElValorTabulado()
    {
        ...
        var mxConservador = qLimite * aLimite * aLimite / 2.0; // q*a^2/2 (cantiléver uniforme, observación año de revisión)
        ...
        // observación año de revisión: la cota corregida q*a^2/2 (cantiléver UNIFORME) también debe
    ...
    // ---- Observación N3/R3 (auditoría externa tercera ronda, año de revisión): la cota conservadora ahora
    ...
    public void FueraDeDominio_RegimenLargoBajo_AcotaMyConTablaEnBSobreA4_NoConLaFranjaSinAcotar()
    {
        ...
        AssertTol("Exterior (b/a=2.96): Mx = q*a^2/2 (cantiléver UNIFORME, observación año de revisión)", sismico.Exterior.MxPosGobernanteKNmM, mxEsperado, atol: 1e-6);
        ...
        // Espejo para el cortante lateral (observación menor de la cuarta ronda de auditoría): en el
    ...
    public void FueraDeDominio_BandaIntermedia_UnoPuntoCeroHastaDosPuntoCero_AcotaConFilaBSobreA3()
    {
        ...
        // hueco de grilla de esa fila (b/a<2.0 solo tabula c/a=[1.0,0.5]). REFINAMIENTO año de revisión:
        ...
        AssertTol("Banda 1.0<=b/a<2.0: My = min(q*L^2/8, |My| tabulado (3.0,c/a)) [refinamiento año de revisión]",
        ...
        AssertTol("Banda 1.0<=b/a<2.0: V lateral = min(q*L/2, V tabulado (3.0,c/a)) [refinamiento año de revisión]",
            sismico.Exterior.VSideMaxKNm, vEsperado, atol: 1e-6);
        var mxTab = Math.Max(Math.Abs(tab.MxPosGobernanteKNmM), Math.Abs(tab.MxNegGobernanteKNmM));
        AssertTol("Banda 1.0<=b/a<2.0: Mx = min(q*a^2/2, |Mx| tabulado (3.0,c/a)) [refinamiento año de revisión]",

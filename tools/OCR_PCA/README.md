...
**sesión año de revisión** para extraer los valores publicados del **"Example 1 — Rectangular Tank
...
- Con esta herramienta se extrajo el texto de las páginas del Ejemplo 1 (págs. físicas 173-198) y
  se verificó la comparación registrada en `la hoja de seguimiento del proyecto` (entrada
  año de revisión). Los textos OCR quedaron conservados como evidencia en `pca_ocr/`.
...
- **Patch a `is-electron`** (lo aplica `setup.js`): el entorno de desarrollo de sesión ejecuta Node dentro de un
...
- Para coeficientes de diseño ya hay datos **CONFIRMADOS** en
  ...
  no un OCR nuevo.
- Esta herramienta es auxiliar de verificación normativa (leer el manual), no un referencia autoritativa única:

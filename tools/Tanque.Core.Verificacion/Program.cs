...
// la lista blanca de red del entorno de desarrollo, así que el proyecto de pruebas xUnit
...
// de ese entorno de desarrollo. En un equipo con acceso normal a NuGet, use "dotnet test" sobre
...
Console.WriteLine();
Console.WriteLine("=== Ejercicio 2: tanque de lados dispares, con tapa (Tabla 36 de la referencia) ===");
{
...
Console.WriteLine("=== Fuerza dinámica de suelo (Mononobe-Okabe) -- ejercicio 2 (Tabla 39) ===");
{
    ...
    // Correccion año de revisión (auditoria externa del usuario, "revision sismo fuera de dominio PCA
    // Caso 7", observación P5): Qae debe escalar con Hm (altura de suelo RETENIDO contra el muro), NO
    ...
    Console.WriteLine("-- Correccion Ht->Hm (observación P5, auditoria año de revisión): Qae debe escalar con Hm, no con Ht --");
...
Console.WriteLine("=== Diseño a flexión, cortante y control de fisuración (F.12-14) -- ejercicio 2 ===");
{
    ...
    // CORREGIDO año de revisión (auditoría externa, segunda ronda, H-CRÍTICO-1/H-ALTO-2): el signo de
    ...
    Console.WriteLine("-- Diseño a flexión (sección 8.1) -- verificación INDEPENDIENTE (forma cerrada ACI 318, observación H-CRÍTICO-1/H-ALTO-2) --");
    // Recomendación §7.3 del informe de auditoría externa (segunda ronda, año de revisión): una
...
Console.WriteLine("=== Placas rectangulares PCA/Marcus (F.7, 10-11) -- ejercicio 1, placa de fondo (r=1.0 exacto) ===");
{
    ...
    // publicada por la referencia como "q" para aislar la verificación del mecanismo Marcus en sí.
    ...
    // estándar del proyecto. Se documenta como discrepancia abierta de la referencia, no se fuerza.
    Console.WriteLine($"  [NOTA] V=Cs×q×a={resultado.VxKNm:0.###} kN -- la Tabla 24 publica 8.97 kN (diff ~0.4%, discrepancia abierta de la referencia, no de la fórmula: ver más abajo la coincidencia exacta de las Tablas 26-29 con esta misma fórmula/datos).");
    ...
    void VerificarCampo(string nombreTabla, double[,] campoCalculado, JsonNode tablaJson, (int fila, int col, string motivo)[]? excepciones = null)
    {
        ...
        for (var fila = 0; fila < 6; fila++)
        {
            ...
            for (var col = 0; col < 6; col++)
            {
                ...
                if (actual != esperado)
                {
                    ...
                    {
                        celdasExcepcion++;
                        Console.WriteLine($"  [NOTA] {nombreTabla}[{fila},{col}]: actual={campoCalculado[fila, col]:0.###} (redondeado {actual}), referencia publica {esperado} -- divergencia esperada y documentada: {ex.motivo}");
                    }
                    else
                    {
                        fallos++;
        ...
        Console.WriteLine($"  [OK  ] {nombreTabla}: las 36 celdas (filas TOP..0.5a) coinciden con la referencia tras redondear al entero más cercano{sufijo}.");
    }
    ...
    // confianza ya documentada en tabla_placa_biaxial.json (observación_adicional_Cmx_r1) -- el
    ...
    // de verdad elegida para todo el módulo) por política del proyecto; la referencia, al haber
...
Console.WriteLine("=== Placas rectangulares -- interpolación, rango tabulado y cargas de diseño (F.7, 10-11) ===");
{
    ...
    // Observación 12: la carga de diseño de cubierta corregida usa 1.2×D (no la constante 1.2 sin
    ...
    { fallos++; Console.WriteLine("  [FAIL] U cubierta coincide con el resultado del bug del original (observación 12 no corregido)"); }
    else Console.WriteLine($"  [OK  ] U cubierta ({uCubierta:0.###}) NO reproduce el bug de literal Decimal \"1.2D\" del programa original (que hubiera dado {Math.Max(c1, Math.Max(c2, c3ConBugDelOriginal)):0.###}) -- observación 12 corregido");
}

Console.WriteLine();
Console.WriteLine("=== Placas rectangulares -- MetodoInterpolacion.RedondearSuperior (nota de diseño v2) ===");
{
...
Console.WriteLine("=== Muros rectangulares PCA/Marcus (F.8-9) -- ejercicio 1, r=1.5 exacto ===");
{
    ...
    Console.WriteLine("-- Cortante (Tabla 17) -- condicion 2 (exterior/suelo, Ka corregido -- observación 1) --");
    ...
    // La referencia publico estos valores usando el Ka INCORRECTO (observación 1) -- ver docstring de la
    // clase. Con el Ka corregido que usa Tanque.Core, el resultado NO debe coincidir con la Tabla
    // 17 (discriminador, igual que el observación 12 de Placas), y en su lugar debe coincidir con la
    // version recalculada con Ka correcto.
    var kaCorrecto = presiones1.Ka;
    var q2ConBug = 1.6 * (0.254366 /*Ka incorrecto observación 1, phi=32*/) * Mat(raiz1, "gamma_suelo_kNm3") * Hm;
    totalAserciones++;
    if (Math.Abs(resultado2.VBottomKNm - 23.44) < 0.05)
    { fallos++; Console.WriteLine($"  [FAIL] V fondo condicion 2 coincide con la Tabla 17 (23.44 kN) -- el Ka corregido (observación 1) debia dar un resultado distinto"); }
    else Console.WriteLine($"  [OK  ] V fondo condicion 2 con Ka corregido ({resultado2.VBottomKNm:0.###} kN) difiere de la Tabla 17 (23.44 kN, calculada con el Ka incorrecto del observación 1) -- confirmado: observación 1 no se hereda");
    var resultado2ConBug = MurosRectangulares.Calcular(r2, r2, q2ConBug, Hm, true);
    AssertTol("V fondo condicion 2, RECALCULADO con el Ka incorrecto del observación 1 (discriminador)", resultado2ConBug.VBottomKNm, 23.44, atol: 0.05);
    ...
    VerificarCampoMuro("My (Tabla 21, condicion 1)", resultado1.CampoMy, mom["condicion_1_My_tabla21"]!,
        [(7, 3, "discrepancia aislada de transcripcion de la referencia -- publicado 2.6, calculado 2.967; ninguna otra de las 65 celdas restantes de esta tabla difiere en mas de 0.02 -- ver docstring de MurosRectangulares")]);

    Console.WriteLine("-- Momentos (Tablas 22/23, condicion 2 -- RECALCULADOS con el Ka incorrecto del observación 1, discriminador) --");
    var resultado2ConBugParaMomentos = MurosRectangulares.Calcular(r2, r2, q2ConBug, Hm, true);
    VerificarCampoMuro("Mx (Tabla 22, condicion 2, Ka observación1)", resultado2ConBugParaMomentos.CampoMx, mom["condicion_2_Mx_tabla22"]!);
    VerificarCampoMuro("My (Tabla 23, condicion 2, Ka observación1)", resultado2ConBugParaMomentos.CampoMy, mom["condicion_2_My_tabla23"]!,
...
Console.WriteLine("=== Muros rectangulares -- interpolacion, rango tabulado y combinacion Marcus ===");
{
    ...
    // MetodoInterpolacion.RedondearSuperior (nota de diseño v2): en r=1.125 debe usar el tabulado
    ...
    // una celda fija) -- verificacion estructural, no numerica contra un referencia autoritativa unica (no hay tabla
...
{
    Console.WriteLine("-- Interpolacion exacta en un punto tabulado (b/a=2.0, c/a=1.5, coincide con ejercicio 2 de la referencia) --");
    ...
    // Verificacion estructural (no hay referencia autoritativa unica publicado): el valor interpolado debe caer
    ...
    // para que, con la luz EJE A EJE (corrección año de revisión), sigan dando los ratios tabulados
    ...
    // Diseno a flexion (+ segunda pasada de fisuracion, fix del año de revisión -- ver docstring de
    ...
    Console.WriteLine("-- Segunda pasada de rediseno por fisuracion (fix año de revisión): caso sintetico donde el As de flexion pura NO cumple fisuracion --");
    totalAserciones++;
    {
    ...
    Console.WriteLine("-- Refuerzo minimo de MURO de tanque (cruce normativo C.23-C.14.3, año de revisión): cuantia minima 0.0030, no la generica 0.0018 --");
    ...
    Console.WriteLine("-- CORRECCION año de revisión (auditoria externa, \"revision sismo fuera de dominio PCA Caso 7\"): --");
    ...
    // refinamiento año de revisión acota con la fila tabulada b/a=3.0.
    totalAserciones++;
    ...
    var luzEjeAejeFD = geoFueraDominio.LLargoM - geoFueraDominio.EmEspesorMuroM; // span propio EJE A EJE (corrección año de revisión)
    // REFINAMIENTO año de revisión (banda 1.0<=b/a<2.0): c/a=(B-em)/HL=0.6 cabe en la fila b/a=3.0 y la
    ...
    var mxBaseEsperado = Math.Min(qInteriorFD * aFD * aFD / 2.0, mxTabFD); // cantiliever UNIFORME q*a^2/2 (observación año de revisión), acotado por la fila b/a=3.0
    ...
    totalAserciones++;
    {
        ...
        var mxConservadorLimite = qLimite * aLimite * aLimite / 2.0; // q*a^2/2 (cantiléver uniforme, observación año de revisión)
    ...
    // observación año de revisión (carga del Caso 7 UNIFORME): la cota anterior q*a^2/6 (cantiléver
    ...
    Console.WriteLine("-- CORRECCION año de revisión (auditoria externa tercera ronda, observación N3/R3 \"cota conservadora sin regimen\"): --");
    ...
    if (sismicoRegimenLargo.Exterior is not null && sismicoRegimenLargo.Exterior.EsAproximacionConservadora
        ...
        Console.WriteLine("  [OK  ] Exterior (b/a=2.96,c/a=2.16, eje a eje): EsAproximacionConservadora=true, motivo documenta b/a y c/a, SIN remitir al codigo fuente (observación N1 corregido)");
    ...
    // Verificacion INDEPENDIENTE del recorte de My (observación N3/R3): recalculo en C#, a partir de
    ...
    // "segunda derivacion cerrada que no reutiliza el propio codigo" ya usada para el observación de
    ...
    // Verificacion INDEPENDIENTE del recorte de V (cortante lateral), observación menor de la cuarta
    // ronda de auditoria (año de revisión): el cortante lateral de la cota conservadora usaba la franja
    ...
    Console.WriteLine("-- banda intermedia 1.0<=b/a<2.0 (REFINAMIENTO año de revisión: cota tabulada en b/a=3.0, mínimo con la franja) --");
    ...
    {
        AssertTol("Banda 1.0<=b/a<2.0: Mx = min(q*a^2/2, |Mx| tabulado (3.0,c/a)) [refinamiento año de revisión]", sismicoBandaIntermedia.Exterior.MxPosGobernanteKNmM, Math.Min(mxCantilieverBanda, mxTabBanda), atol: 1e-6);
        AssertTol("Banda 1.0<=b/a<2.0: My = min(q*L^2/8, |My| tabulado (3.0,c/a)) [refinamiento año de revisión]", sismicoBandaIntermedia.Exterior.MyPosGobernanteKNmM, myEsperadaBanda, atol: 1e-6);
    }
    ...
    AssertTol("Banda 1.0<=b/a<2.0: V lateral = min(q*L/2, V tabulado (3.0,c/a)) [refinamiento año de revisión]", sismicoBandaIntermedia.Exterior!.VSideMaxKNm, vEsperadaBanda, atol: 1e-6);
    ...
    // FUNDAMENTO DEL REFINAMIENTO DE BANDA (año de revisión): monotonicidad de |Mx| y |My| de esquina
    ...
    try
    {
        ...
        // Correccion año de revisión (auditoria externa del usuario, observación H2): el motivo NO debe
    ...
    // Observación confirmado año de revisión via prueba real de usuario: TipoTanque.Superficial (Hm=0 por
...
Console.WriteLine("=== Modulo 8 conectado a placas (DisenoPlacas: cubierta y fondo, recubrimiento mixto en fondo) ===");
{
    ...
    // Diseno a flexion (+ segunda pasada de fisuracion, fix del año de revisión) autoconsistente -- ver
...
Console.WriteLine();
Console.WriteLine("=== TipoTanque y Flotabilidad (nota de diseño v2, item 2/6 -- ACI 350.4R-04 §3.1.2) ===");
{
    ...
    Console.WriteLine("-- Materiales.Validar(): GammaSueloSaturadoKNm3, nueva regla (nota de diseño v2, sobreancho) --");
    totalAserciones++;
    ...
    // Corrección año de revisión (Opción A, PCA pág. 173): la flexión usa la luz EJE A EJE (a=B-em,
    ...
    // Cierre año de revisión (observación del usuario): la envolvente debe propagar el DETALLADO (Ø/s)
    ...
    // nota de diseño v2 (año de revisión, a peticion del usuario tras revisar el reporte real): los diagramas de
...
Console.WriteLine();
Console.WriteLine("=== Espesores minimos de muro NSR-10 C.23-C.14.6 (nota de diseño v2, item 7) ===");
{
...
Console.WriteLine();
Console.WriteLine("=== Sugerencia de ajuste geometrico cuando r cae fuera del rango tabulado PCA (nota de diseño v2, observación de UX año de revisión, quinta continuacion) ===");
{
    ...
    // Reproduccion EXACTA de la geometria reportada por el usuario (año de revisión, quinta continuacion):
...
Console.WriteLine();
Console.WriteLine("=== Modulo 12 (nota de diseño v3, Fase A): CalculadorTanque + Tanque.Reportes.GenerarReporte ===");
{
    // observaciones H1/H3 del informe de auditoria externa del usuario (año de revisión): la orquestacion
    ...
    // Tanque.App.MainWindow.EjecutarCalculo (tal como existia el año de revisión antes de esta
    ...
    // nota de diseño v3, Fase B (año de revisión, decidida por el usuario: secciones agrupadas Expander).
    ...
    // Fase 1 del frente de interfaz (año de revisión) -- veredicto global CUMPLE/NO CUMPLE
    ...
    // Fase 2 del frente de interfaz (año de revisión) -- curvas de momento (faja gobernante) y cortante
    ...
    // Fase3 del frente de interfaz (año de revisión) -- reporte profesional HTML (ReporteHtml.cs).
    ...
    // balanceadas (smoke-test de render sin dependencias de navegador).
    {
        ...
        // (i) Orientación de los mapas de calor (año de revisión): nota "cómo leer", leyenda semántica
    ...
    // Fase4 (año de revisión) -- persistencia JSON (PersistenciaTanque): round-trip del escenario F12
    ...
    // Fase4 (año de revisión) -- exportación CSV (ExportadorCsv, ítem 2): grillas/resultados en formato
...
// ============================================================================
// Pendiente "C" (año de revisión) -- catálogo real de barras (diámetros/separaciones
// comerciales) y detallado C.23-C.14.3 (doble capa >250mm, espaciamiento máx. 300mm).
// ============================================================================
{
    Console.WriteLine("=== Catalogo de barras y detallado C.23-C.14.3 (pendiente C, nota de diseño) ===");
...
// ============================================================================
// Cuantía mínima de retracción/temperatura (año de revisión) -- cruce normativo
...
// ============================================================================
{
    ...
    //    mínima de la tabla en la dirección de su refuerzo -- ya no la genérica 0.0018.
    {
        ...
        // Notas opcionales C.23-C.7.12.2.1 (año de revisión): 50 % en la cara inferior (contra el suelo).
...
// ============================================================================
// Diámetro de barra por elemento + blindaje normativo (año de revisión) -- cierre de
...
// con separaciones amplias que la norma admite). Cruce normativo año de revisión.
// ============================================================================
{
...
// ============================================================================
// C.23-C.10.5/C.10.6 contra placas (cruce normativo año de revisión, cierre del punto 13(b) de
...
// DisenoPlacas+CatalogoBarras, cruce año de revisión); C.23-C.10.5.3 (excepción As >= 1.33 x As,análisis
